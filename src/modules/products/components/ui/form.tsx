...
export const ProductForm = ({
  ...
  const { mutateAsync } = useGenerateProductMutation();
  ...
  const handleGenerateProduct = async () => {
    ...
    const toastId = toast.loading("Generating by AI...");

    try {
      const res = await mutateAsync();

      if (res.isSuccess) {
        toast.dismiss(toastId); // close loading
        toast.success("Generated successfully!");

        form.reset({
          name: res.products.name ?? "",
          body: res.products.body ?? "",
          description: res.products.description ?? "",
          price: res.products.price ?? 0,
          category: res.products.category ?? "",
          tags: res.products.tags?.join(",") ?? "",
        });
      } else {
